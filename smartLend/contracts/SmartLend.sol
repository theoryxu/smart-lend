...
contract smartLend {
	...
	function _checkUncomfirmed() private returns(Lend[]) {

		Lend[] unConfirmedLend; 
		for (uint i; i < lends.length;i++) {

			if (lendToOwner[i].debit == msg.sender && lends[i].isConfirmed == false) {

				//checkLend(i);
				unConfirmedLend.push(lends[i]);
			}

		}

		return unConfirmedLend;
	}

	//借方确认——通过输入y
	function confirmLend(uint i, string isConfirmed) returns(uint) {
        
		if (keccak256(isConfirmed) == keccak256("y")) {
			lends[i].isConfirmed = true;
			return 1;
		}

		if (keccak256(isConfirmed) == keccak256("n")) {
			lends[i].isClosed = true;
			return 2;
		}		
	...
	//检查待还款余额
	function checkLend(uint id) private returns(string creditName, string creditId, string debitName, string debitId, uint monthRate,uint defaultRate, uint totalToPay, uint capitalToPay, uint interestToPay, uint startTime, uint dueTime, uint lastTime, bool isConfirmed, bool isClosed) {
        require(msg.sender == lendToOwner[id].credit || msg.sender == lendToOwner[id].debit);
		creditName = lends[id].creditName;
		creditId = lends[id].creditId;
        debitName = lends[id].debitName;
        debitId = lends[id].debitId;
        monthRate = lends[id].monthRate;
        defaultRate = lends[id].defaultRate;
        totalToPay = lends[id].totalToPay;
        capitalToPay = lends[id].capitalToPay;
        interestToPay = lends[id].interestToPay;
        startTime = lends[id].startTime;
        dueTime = lends[id].dueTime;
        lastTime = lends[id].lastTime;
        isConfirmed = lends[id].isConfirmed;
        isClosed = lends[id].isClosed;
